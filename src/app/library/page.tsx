...
function EntryCard({
  ...
  return (
    <button
      onClick={onToggle}
      className="group w-full rounded-xl border border-zinc-200 bg-zinc-50 p-3.5 text-left transition-all hover:border-indigo-500/30 hover:bg-zinc-100 dark:border-zinc-700/50 dark:bg-zinc-800/30 dark:hover:bg-zinc-800/50"
    >
      ...
      {isExpanded ? (
        <div className="mt-2">
          <MathText className="text-xs leading-relaxed text-zinc-600 dark:text-zinc-400">
            {entry.statement}
          </MathText>
          {entry.notes && (
            <p className="mt-2 text-[11px] italic text-zinc-400 dark:text-zinc-500">
              {entry.notes}
            </p>
          )}
        </div>
